...
class _PerfilScreenState extends State<PerfilScreen> {
  ...
  Future<void> _verificarSessao() async {
    ...
    if (mounted) setState(() { _logado = logado; _nomeCliente = nome; _emailCliente = email; _carregando = false; });
  }
  ...
  Future<void> _login() async {
    ...
    if (res != null && res['token'] != null) {
      ...
      setState(() { _erro = 'E-mail ou senha incorretos.'; _processando = false; });
    }
  ...
  Future<void> _cadastrar() async {
    ...
    if (res != null && res['token'] != null) {
      ...
      setState(() { _erro = res?['erro'] ?? 'Erro ao criar conta. Tente outro e-mail.'; _processando = false; });
    }
    if (mounted) setState(() => _processando = false);
  }
  ...
  Future<void> _logout() async {
    ...
    await _verificarSessao();
    setState(() { _emailCtrl.clear(); _senhaCtrl.clear(); _nomeCtrl.clear(); });
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          Text(_nomeCliente ?? '', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          Text(_emailCliente ?? '', style: const TextStyle(fontSize: 14, color: Colors.grey)),
          const SizedBox(height: 32),
          _itemMenu(Icons.shopping_bag_outlined, 'Meus Pedidos', corPrimaria, () {}),
          _itemMenu(Icons.location_on_outlined, 'Meus Endereços', corPrimaria, () {}),
  ...
  Widget _itemMenu(IconData icon, String label, Color cor, VoidCallback onTap) {
    return Container(
        ...
        children: [
          ...
          Container(
            ...
            child: Text(
              ...
              style: TextStyle(fontSize: 14, color: corPrimaria.withOpacity(0.9), height: 1.5),
            ),
          ),
          ...
          Row(
            children: [
              Expanded(
                child: _botaoOpcao('Entrar', !_mostrarCadastro, corPrimaria, () => setState(() { _mostrarCadastro = false; _erro = ''; })),
              ),
              const SizedBox(width: 10),
              Expanded(
                child: _botaoOpcao('Criar conta', _mostrarCadastro, corPrimaria, () => setState(() { _mostrarCadastro = true; _erro = ''; })),
              ),
            ],
          ),
          ...
          if (_mostrarCadastro) ...[
            ...
            _campo('Telefone', _telefoneCtrl, false, tipo: TextInputType.phone),
          ],
          _campo('E-mail *', _emailCtrl, false, tipo: TextInputType.emailAddress),
          const SizedBox(height: 14),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: _processando ? null : (_mostrarCadastro ? _cadastrar : _login),
              ...
              child: _processando
                  ...
                  : Text(_mostrarCadastro ? 'Criar conta' : 'Entrar', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            ),
          ),
        ],
      ),
  ...
  Widget _campo(String label, TextEditingController ctrl, bool senha, {TextInputType tipo = TextInputType.text}) {
    return TextField(
      controller: ctrl,
      obscureText: senha,
      keyboardType: tipo,
  ...
  Widget _botaoOpcao(String label, bool ativo, Color cor, VoidCallback onTap) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Text(label, textAlign: TextAlign.center, style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: ativo ? Colors.white : cor)),
      ),
    );
  }
}
