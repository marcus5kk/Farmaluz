...
class ApiService {
  ...
  static Future<Map<String, dynamic>?> getFarmacia() async {
    try {
      final res = await http.get(Uri.parse('$_base/farmacia'), headers: _headers);
  ...
  static Future<List<Categoria>> getCategorias() async {
    try {
      final res = await http.get(Uri.parse('$_base/categorias'), headers: _headers);
  ...
  static Future<List<BannerItem>> getBanners() async {
    try {
      final res = await http.get(Uri.parse('$_base/banners'), headers: _headers);
      if (res.statusCode == 200) {
        final List data = jsonDecode(res.body);
  ...
  static Future<List<Produto>> getProdutos({
    ...
    try {
      ...
      if (codigoBarras != null && codigoBarras.isNotEmpty) params['codigo_barras'] = codigoBarras;
      final uri = Uri.parse('$_base/produtos').replace(queryParameters: params);
  ...
  static Future<Map<String, dynamic>?> cadastrar({
    ...
    try {
      final res = await http.post(
        ...
        headers: _headers,
        body: jsonEncode({'nome': nome, 'email': email, 'senha': senha, if (telefone != null) 'telefone': telefone}),
      );
  ...
  static Future<void> salvarSessao(String token, Map<String, dynamic> cliente) async {
    ...
    await prefs.setInt('id_cliente', int.tryParse(cliente['id'].toString()) ?? 0);
  }
